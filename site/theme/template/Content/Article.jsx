...
export default class Article extends React.PureComponent {
  ...
  render() {
    ...
    const locale = this.context.intl ? this.context.intl.locale : 'zh-CN';
    ...
    return (
      <DocumentTitle title={`${title[locale] || title} - Ant Design`}>
        <article className="markdown" ref={(node) => { this.node = node; }}>
          ...
          {
            (!content.toc || content.toc.length <= 1 || meta.toc === false) ? null : <Affix className="toc-affix" offsetTop={16}>
              {
